...
class StephanDiscriminator(nn.Module):
    ...
    def __init__(self, nf, ndown, nres, batch_norm=False, sigmoid=False, conditional=True, spectral_norm=True):
        ...
        self.downblocks = []
        nf_in = 2 if self.upsample else 1;
        nf_out = nf // 2 ** (ndown - 1)
...
class DSDiscriminatorSmoothed(nn.Module):
    def __init__(self, channels_img, features_d, num_classes, img_size):
        ...
        self.embed = nn.Sequential(
            # Input: N x channels_noise x 16 x 16
            self._expand_block(channels_img, features_d * 16, kernel_size=3, stride=1, padding=1, scale_factor=1),
            # img: 16x16
            self._expand_block(features_d * 16, features_d * 8, kernel_size=3, stride=1, padding=1, scale_factor=2),
            # img: 32x32
            self._expand_block(features_d * 8, features_d * 4, kernel_size=3, stride=1, padding=1, scale_factor=2),
            # img: 64x64
...
class DSGeneratorSmoothed(nn.Module):
    def __init__(self, noise_shape, channels_img, features_g, num_classes, img_size, embed_size):
        ...
        self.net = nn.Sequential(
            # Input: N x channels_noise x 16 x 16
            self._block(noise_shape[0] + embed_size, features_g * 16, kernel_size=3, stride=1, padding=1,
                        scale_factor=1),  # img: 16x16
            self._block(features_g * 16, features_g * 8, kernel_size=3, stride=1, padding=1, scale_factor=2),
            # img: 32x32
            self._block(features_g * 8, features_g * 4, kernel_size=3, stride=1, padding=1, scale_factor=2),
            # img: 64x64
            nn.UpsamplingBilinear2d(scale_factor=2),
            nn.Conv2d(
...
class LeinDisc(nn.Module):
    def __init__(self, input_channels=1, nonlin='leaky_relu'):
        ...
        self.hr_block2 = nn.Sequential(LeinResBlock(in_planes=256, planes=256, stride=1,
                                                    nonlin=nonlin))  # , block(in_planes=256, planes=256, stride=1, nonlin = nonlin))
        self.lr_block2 = nn.Sequential(LeinResBlock(in_planes=512, planes=256, stride=1,
                                                    nonlin=nonlin))  # ,block(in_planes=256, planes=256, stride=1, nonlin = nonlin))
...
class LeinDisc2(nn.Module):
    def __init__(self, input_channels=1, nonlin='leaky_relu', norm=None):
        ...
        self.hr_block1 = nn.Sequential(*hr_block)
        self.lr_block1 = nn.Sequential(*lr_block)
        self.hr_block2 = nn.Sequential(LeinResBlock2(in_planes=256, planes=256, stride=1, nonlin=nonlin,
                                                     norm=norm))  # , block(in_planes=256, planes=256, stride=1, nonlin = nonlin))
        self.lr_block2 = nn.Sequential(LeinResBlock2(in_planes=512, planes=256, stride=1, nonlin=nonlin,
                                                     norm=norm))  # ,block(in_planes=256, planes=256, stride=1, nonlin = nonlin))
        self.dense1 = nn.Linear(512, 256)
        self.dense2 = nn.Linear(256, 1)
        nn.init.kaiming_normal_(self.dense1.weight, nonlinearity='leaky_relu')
...
class WGANGP(LightningModule):
    def __init__(self, generator, discriminator, noise_shape, channels_img, features_g, num_classes, img_size,
                 embed_size, features_d,
    ...
    def configure_optimizers(self):
        ...
        return [{"optimizer": disc_opt, "frequency": self.disc_freq},
                {"optimizer": gen_opt, "frequency": self.gen_freq}]
...
class CheckCorrector(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 100 == 0:
            ...
            self.log('our_error_minus_forecast_error', l1error - forecast_l1error, on_epoch=True, on_step=True,
